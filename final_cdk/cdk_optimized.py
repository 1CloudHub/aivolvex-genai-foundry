    ...
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        # Main bucket for knowledge base data
            ...
            frontend_bucket = s3.Bucket(
                ...
                block_public_access=s3.BlockPublicAccess.BLOCK_NONE,  # Allow public access for website hosting
                cors=[s3.CorsRule(  # Optional: Add CORS if needed for web apps
        ...
        # /chat_api endpoint
            ...
            'mkdir startingggggg',
            'mkdir final',
            # Export Lambda environment variables to EC2
            f'export WEBSOCKET_ENDPOINT="{websocket_url}"',
            f'export WEBSOCKET_REGION="{self.region}"',
            f'export RDS_SECRET_NAME="rds-credentials-{rds_name_key}"',
            f'export RDS_SECRET_ARN="{db_instance.secret.secret_arn if db_instance.secret else ""}"',
            f'export RDS_ENDPOINT="{db_instance.instance_endpoint.hostname}"',
            f'export db_host="{db_instance.instance_endpoint.hostname}"',
            f'export SOCKET_ENDPOINT="https://{api.rest_api_id}.execute-api.{self.region}.amazonaws.com/dev/"',
            # Export API Gateway names to EC2
            f'export GENAI_API_NAME="genaifoundry-api{name_key}"',
            f'export COACHING_API_NAME="coaching_assist_voiceops{name_key}"',
            f'export GENAI_API_ID="{api.rest_api_id}"',
            f'export COACHING_API_ID="{coaching_api.rest_api_id}"',
            # Export WebSocket API information to EC2
            f'export WEBSOCKET_API_NAME="GenAIFoundry_ws{name_key}"',
            f'export WEBSOCKET_API_ID="{websocket_api.api_id}"',
            f'export WEBSOCKET_STAGE_URL="{websocket_stage.url}"',
            f'export WEBSOCKET_LAMBDA_NAME="ws_{lambda_name_key}"',
            # Make environment variables persistent by adding to /etc/environment
            f'echo "WEBSOCKET_ENDPOINT={websocket_url}" >> /etc/environment',
            f'echo "WEBSOCKET_REGION={self.region}" >> /etc/environment',
            f'echo "RDS_SECRET_NAME=rds-credentials-{rds_name_key}" >> /etc/environment',
            f'echo "RDS_SECRET_ARN={db_instance.secret.secret_arn if db_instance.secret else ""}" >> /etc/environment',
            f'echo "RDS_ENDPOINT={db_instance.instance_endpoint.hostname}" >> /etc/environment',
            f'echo "db_host={db_instance.instance_endpoint.hostname}" >> /etc/environment',
            f'echo "SOCKET_ENDPOINT=https://{api.rest_api_id}.execute-api.{self.region}.amazonaws.com/dev/" >> /etc/environment',
            f'echo "GENAI_API_NAME=genaifoundry-api{name_key}" >> /etc/environment',
            f'echo "COACHING_API_NAME=coaching_assist_voiceops{name_key}" >> /etc/environment',
            f'echo "GENAI_API_ID={api.rest_api_id}" >> /etc/environment',
            f'echo "COACHING_API_ID={coaching_api.rest_api_id}" >> /etc/environment',
            f'echo "WEBSOCKET_API_NAME=GenAIFoundry_ws{name_key}" >> /etc/environment',
            f'echo "WEBSOCKET_API_ID={websocket_api.api_id}" >> /etc/environment',
            f'echo "WEBSOCKET_STAGE_URL={websocket_stage.url}" >> /etc/environment',
            f'echo "WEBSOCKET_LAMBDA_NAME=ws_{lambda_name_key}" >> /etc/environment'
